...
public class LocatorMapItem extends NetworkSyncedItem {
        ...
        if (world.isClient) {
            ...
            if(structureId != null) {
                ...
                if(strucPos != null) {
                    ...
                    if(map.getOrDefault(LocatorMaps.DIMENSION_KEY_COMPONENT, "minecraft:overworld").equals("minecraft:overworld")) {
                        FilledMapItem.fillExplorationMap(serverWorld, map);
                    } else {
                        fillTerrain(serverWorld, map);
                    }
            ...
            if (world.getRegistryKey() == mapState.dimension) {
                ...
                for (int dz = 0; dz < 128; dz++) {
                    for (int dx = 0; dx < 128; dx++) {
                        RegistryEntry<Biome> registryEntry = world.getBiome(mutable.set((startX + dx) * mapScale, 0, (startZ + dz) * mapScale));
                        bls[dz * 128 + dx] = registryEntry.isIn(BiomeTags.WATER_ON_MAP_OUTLINES);
                    }
                }
    ...
    private static boolean isSeaOrSky(boolean[] bls, int i, int i1) {
